...
void takeSample(void){
  ...
  newSample = true;  
    ...
    //filter1_verdi = filter1_verdi + sample/*/N_filter1*/ - samples_inn[filter1_hode]/*/N_filter1*/;
    //filter1_middelverdi = filter1_middelverdi + abs(filter1_verdi)/*/N_filter1*/ - abs(filter1[filter1_hode])/*/N_filter1*/;
    //sett_inn(&filter1[0], &N_filter1, &filter1_hode, filter1_verdi);
    
    
    //Serial.print("s");
    //Serial.print(5*middelverdi(&samples_inn[0], antall_samples));
    //Serial.print(" ");
    //Serial.print("\n");

    /*
void sett_inn(int *array_inn, int lengde, int nytt_tall){
  for (int i = 0; i < lengde; i++) {
    *(array_inn + lengde - i - 1) = *(array_inn + lengde - i - 2);
  }
  *array_inn = nytt_tall;
}

int middelverdi(int *array_inn, int lengde) {
  long verdi = 0;
  for (int i = 0; i < lengde; i++) {
    verdi += abs(*(array_inn + i));
  }
  return verdi / lengde;
}

int FIR_filter(int *array_inn, int lengde) {
  long verdi = 0;
  for (int i = 0; i < lengde; i++) {
    verdi += *(array_inn + i);
  } 
...
/*void sett_inn(int *array_inn, int *lengde, int *hode, int nytt_tall){
  *(array_inn + *hode) = nytt_tall;
...
/*
    ...
    if (counter > sampling_frekvens) {
      Serial.print(state);
      Serial.print(" ");
      Serial.print(ubestemt_counter);
      Serial.print(" ");
      Serial.print(f0_counter);
      Serial.print(" ");
      Serial.print(f1_counter);
      Serial.print(" ");
      Serial.print(filter0_sammenligning);
      Serial.print(" ");
      Serial.println(filter1_sammenligning);
...
//long samples_middelverdi = 0;
//samples_middelverdi = samples_middelverdi + abs(sample) - abs(samples_inn[samples_hode]);
